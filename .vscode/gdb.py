...
import sys
import traceback
__file__ = r"C:\Users\Lity\Desktop\workspace\go-interpreter\.vscode\gdb.py"
...
class DollarPrintFunction(gdb.Function):
    ...
    def invoke_static(arg):
        ...
        if (
            arg.type.code == gdb.TYPE_CODE_PTR
            and arg.type.target().name == "NimStringDesc"
        ):
            return arg
        ...
        for func, arg_typ in DollarPrintFunction.dollar_functions:
            ...
            if arg_typ == argTypeName:
                func_value = gdb.lookup_global_symbol(
                    func, gdb.SYMBOL_FUNCTIONS_DOMAIN
                ).value()
...
class NimStringEqFunction(gdb.Function):
    ...
    def invoke_static(arg1, arg2):
        if (
            arg1.type.code == gdb.TYPE_CODE_PTR
            and arg1.type.target().name == "NimStringDesc"
        ):
            str1 = NimStringPrinter(arg1).to_string()
        else:
            str1 = arg1.string()
        if (
            arg2.type.code == gdb.TYPE_CODE_PTR
            and arg2.type.target().name == "NimStringDesc"
        ):
...
class DollarPrintCmd(gdb.Command):
    ...
    def __init__(self):
        super(DollarPrintCmd, self).__init__(
    ...
    def invoke(self, arg, from_tty):
        ...
        if strValue:
            gdb.write(NimStringPrinter(strValue).to_string() + "\n", gdb.STDOUT)

        # could not find a suitable dollar overload. This here is the
        # fallback to get sensible output of basic types anyway.

        elif (
            param.type.code == gdb.TYPE_CODE_ARRAY
            and param.type.target().name == "char"
        ):
...
class KochCmd(gdb.Command):
    ...
    def __init__(self):
        ...
        self.binary = os.path.join(os.path.dirname(os.path.dirname(__file__)), "koch")
...
class NimbleCmd(gdb.Command):
    ...
    def __init__(self):
        super(NimbleCmd, self).__init__(
            "nimble", gdb.COMMAND_USER, gdb.COMPLETE_FILENAME
        )
        self.binary = os.path.join(
