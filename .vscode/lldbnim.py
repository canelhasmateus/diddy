...
def NimSeq_SummaryFormatter(valobj, internal_dict):
    ...
    try:
        ...
        s = (
            "{ len="
            + str(len.GetValueAsSigned())
            + ", reserved="
            + str(reserved.GetValueAsSigned())
            + " }"
        )
    except:
        pass
        ...
        try:
            ...
            for i in range(self.num_elem_children()):
                ...
                if hash != 0:
                    ...
                    field2 = elem.GetChildMemberWithName(
                        "Field2"
                    ).GetNonSyntheticValue()
                    value = field2.CreateValueFromAddress(
                        key, field2.GetLoadAddress(), field2.GetType()
                    )
...
def myFormatter(valobj, internal_dict):

    s = "error!"
    try:
        nonSynth = valobj.GetNonSyntheticValue()

        print(nonSynth.GetTypeName())
        print(nonSynth.GetDisplayTypeName())


        print(nonSynth.GetName()) # variable name
        # print(nonSynth.GetByteSize())
        # print(nonSynth.GetFormat())
        # print(nonSynth.GetValueType())

        # print(nonSynth.GetLocation()) # address
        # print(nonSynth.GetTypeSummary())
        # print(nonSynth.GetTypeFilter()) # nome da função de print
        # print(nonSynth.GetType())
        # print(nonSynth.GetDeclaration()) # souce filename:line
        # print(nonSynth.TypeIsPointerType())

        print(valobj.__dir__())
        s = re.findall( '(?:^tyObject_)(.+?)(?:colonObjectType)*(?:_.+)$' , nonSynth.GetTypeName())[0]
        print( s )
    except:
        pass
...
def __lldb_init_module(debugger, internal_dict):
    ...
    category.AddTypeSummary(
        ...
        lldb.SBTypeSummary.CreateWithFunctionName(
            "lldbnim.NimStringDesc_SummaryFormatter"
        ),
    )
    ...
    category.AddTypeSynthetic(
        ...
        lldb.SBTypeSynthetic.CreateWithClassName(
            "lldbnim.NimSeqProvider", lldb.eTypeOptionCascade
        ),
    )
    ...
    category.AddTypeSynthetic(
        lldb.SBTypeNameSpecifier("^tyObject_Table__.*$", True),
        lldb.SBTypeSynthetic.CreateWithClassName(
            "lldbnim.NimTableProvider", lldb.eTypeOptionCascade
        ),
    )

    # category.AddTypeSummary(
    #     lldb.SBTypeNameSpecifier("^tyObjec.*$", True),
    #     lldb.SBTypeSummary.CreateWithFunctionName("lldbnim.myFormatter"),
    # )
